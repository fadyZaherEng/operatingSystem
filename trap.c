...
//---------------------------------------------------------------------------
	...
	// allocate,map,copy
	struct  Frame_Info * Free_Allocate=NULL;
	if(allocate_frame(&Free_Allocate)!=E_NO_MEM) map_frame(curenv->env_page_directory,Free_Allocate,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
	if(pf_read_env_page(curenv,(void*)fault_va)==E_PAGE_NOT_EXIST_IN_PF){
	if(fault_va>=USER_HEAP_MAX&& fault_va<USTACKTOP) pf_add_empty_env_page(curenv,fault_va,0);//check stack
	else
    panic("Error");
    }

	// placement in working set
	...
	curenv->page_last_WS_index++;
	 ...
	 //Part Of Page Replacement Algorithm LRUTimeStamp-----------------------------
	 else if(isPageReplacmentAlgorithmLRUTimeStamp()){
	 uint32 min_time_stamp=env_page_ws_get_time_stamp(curenv,0);//get time of first
	 int WS_idx=0;
	 for(int i=1;i<curenv->page_WS_max_size;i++){
	 uint32 x=env_page_ws_get_time_stamp(curenv,i);
	    if(min_time_stamp>x){
	        WS_idx=i;
			min_time_stamp=x;
		}
	 }
	 uint32 Fault_Victim_Page=curenv->ptr_pageWorkingSet[WS_idx].virtual_address;
	 //update empty
	 curenv->ptr_pageWorkingSet[WS_idx].empty=0;
	 curenv->ptr_pageWorkingSet[WS_idx].virtual_address=fault_va;
	 //Set time stamp
	 env_page_ws_set_time_stamp(curenv,WS_idx);
	 uint32* Page_Table=NULL;
	 get_page_table(curenv->env_page_directory,(void*)Fault_Victim_Page,&Page_Table);
	 if((Page_Table[PTX(Fault_Victim_Page)])&&PERM_MODIFIED==PERM_MODIFIED){
				 struct Frame_Info *Modified_Frame=NULL;
			     Modified_Frame=get_frame_info(curenv->env_page_directory,(void*)Fault_Victim_Page,&Page_Table);
			     pf_update_env_page(curenv,(void*)Fault_Victim_Page, Modified_Frame);
	 }
	unmap_frame(curenv->env_page_directory,(void*)Fault_Victim_Page);
	 struct Frame_Info *Free_Allocate=NULL;
	 if(allocate_frame(&Free_Allocate)!=E_NO_MEM){
	 map_frame(curenv->env_page_directory,Free_Allocate,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
	  if(pf_read_env_page(curenv,(void*)fault_va)==E_PAGE_NOT_EXIST_IN_PF){
		if(fault_va>=USER_HEAP_MAX&& fault_va<USTACKTOP) pf_add_empty_env_page(curenv,fault_va,0);//check stack
		else
		    panic("Error");
	...
	//Part Of Page Replacement Algorithm FIFO-----------------------------
	else if(isPageReplacmentAlgorithmFIFO()){
    //check size of working set
    uint32 Fault_Victim_Page;
    if(curenv-> page_last_WS_index==curenv->page_WS_max_size) curenv->page_last_WS_index=0;
    //update working set
    Fault_Victim_Page=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
	curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty=0;
    curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address=fault_va;
    curenv->page_last_WS_index++;
    uint32* Page_Table=NULL;
    //update on page file
    get_page_table(curenv->env_page_directory,(void*)Fault_Victim_Page,&Page_Table);
    struct Frame_Info *Free_Allocate=NULL,*Modified_Frame=NULL;
    if((Page_Table[PTX(Fault_Victim_Page)])&&PERM_MODIFIED==PERM_MODIFIED){ Modified_Frame=get_frame_info(curenv->env_page_directory,(void*)Fault_Victim_Page,&Page_Table);pf_update_env_page(curenv,(void*)Fault_Victim_Page, Modified_Frame);}
    unmap_frame(curenv->env_page_directory,(void*)Fault_Victim_Page);
    // allocate , map , copy fault page
    if(allocate_frame(&Free_Allocate)!=E_NO_MEM)
    	map_frame(curenv->env_page_directory,Free_Allocate,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
    if(pf_read_env_page(curenv,(void*)fault_va)==E_PAGE_NOT_EXIST_IN_PF){
    	if(fault_va>=USER_HEAP_MAX&& fault_va<USTACKTOP) pf_add_empty_env_page(curenv,fault_va,0);//check stack
	...
	//Part Of Page Replacement Algorithm CLOCK-----------------------------
	else if(isPageReplacmentAlgorithmCLOCK()){
	while(5){//check size of working set
	if(curenv->page_last_WS_index==curenv->page_WS_max_size) curenv->page_last_WS_index=0;
	for( ;curenv->page_last_WS_index<curenv->page_WS_max_size;curenv->page_last_WS_index++){
		...
		get_page_table(curenv->env_page_directory,(void*)Fault_Victim_Page,&Page_Table);
        //check used
	if((Page_Table[PTX(Fault_Victim_Page)]&PERM_USED)!=PERM_USED){
		//update working set
		curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].empty=0;
		curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address=fault_va;
	    curenv->page_last_WS_index++;
	 ...
	 unmap_frame(curenv->env_page_directory,(void*)Fault_Victim_Page);
	 get_page_table(curenv->env_page_directory,(void*)fault_va,&Page_Table);
	 if(allocate_frame(&Free_Allocate)!=E_NO_MEM){
		 map_frame(curenv->env_page_directory,Free_Allocate,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
		 if(pf_read_env_page(curenv,(void*)fault_va)==E_PAGE_NOT_EXIST_IN_PF){
		 	if(fault_va>=USER_HEAP_MAX&& fault_va<USTACKTOP) pf_add_empty_env_page(curenv,fault_va,0);//check stack
	 ...
	 Page_Table[PTX(fault_va)]=Page_Table[PTX(fault_va)]|PERM_USED;
	 return;
	}
	if((Page_Table[PTX(Fault_Victim_Page)]&PERM_USED)==PERM_USED) Page_Table[PTX(Fault_Victim_Page)]=Page_Table[PTX(Fault_Victim_Page)]&~PERM_USED;
	    }
	  }
	}
